...
###################################################
gr_by_chrom <- lapply(seqlevels(gal), function(seqname)
{
  ...
  gr <- GRanges(Rle(seqname, nrow(gr_mcols)), IRanges(as.integer(names(nread)), width =
                                                        1))
  mcols(gr) <- gr_mcols
...
###################################################
qseq_on_ref_id2 <- endoapply(qseq_on_ref_id, function(ids)
  ids[countMatches(ids, ids) >= 0.2 * length(ids)])
...
qseq_on_ref2 <- relist(unlist(qseq_on_ref, use.names = FALSE)[tmp], qseq_on_ref_id2)
...
cm_by_chrom <- lapply(names(qseq_pos_by_chrom), function(seqname)
  consensusMatrix(
    qseq_on_ref2_by_chrom[[seqname]],
    as.prob = TRUE,
    shift = qseq_pos_by_chrom[[seqname]] -
      1,
...
###################################################
cs_by_chrom <- lapply(cm_by_chrom, function(cm) {
...
set.seed(55)
my_var <- RleList(lapply(seqlengths(Scerevisiae), function(seqlen) {
...
###################################################
bins <- tileGenome(seqinfo(Scerevisiae),
                   tilewidth = 100,
